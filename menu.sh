#!/bin/bash
# VSProxy Manager - menu interativo de portas (estilo RustyManager)

PROXY_BIN="/opt/vsproxy/proxy"
SERVICE_PREFIX="vsproxy-"
DEFAULT_TARGET="127.0.0.1:22"
DEFAULT_STATUS="@VSProxy"
...
open_port() {
    ...
    if [ -f "/etc/systemd/system/${service}" ]; then
        echo "Essa porta já está aberta pelo VSProxy."
        sleep 2
        return
    fi
...
close_port() {
    ...
    if [ ! -f "/etc/systemd/system/${service}" ]; then
        echo "Essa porta não está aberta pelo VSProxy."
        sleep 2
        return
    fi
